...
<body>
    ...
    $m = 0;
    $n = 0;
    if (isset($_REQUEST["M"]) and isset($_REQUEST["N"])) {
        $m = $_REQUEST["M"];
        $n = $_REQUEST["N"];
        session_unset();
        $_SESSION["m"] = $m;
    ...
    if (isset($_SESSION["m"]) and isset($_SESSION["n"])) {
        $m = $_SESSION["m"];
        $n = $_SESSION["n"];
    }
    ?>
    ...
    <form method="post" action="">
        ...
        <input type="number" id="M" name="M" value="<?= $m ?>"><br><br>
        <label for="cols">Enter Number of Columns:</label>
        <input type="number" id="N" name="N" value="<?= $n ?>"><br><br>
        <input type="submit" value="Generate Matrices" name="CreateMatrixBtn">
    </form>
    ...
    if (isset($_REQUEST["mulMat"])) {
        if ($m != $n) {
            echo "<h3> Wrong Dimenstions, cannot multiply matrices </h3>";
        } else {
            echo "<form method=\"post\">";
            $a = $_REQUEST["a"];
            $b = $_REQUEST["b"];
